...
## v1.5 新增功能
- [x] 首页 Top 5 提示词排行：统计每条提示词的使用次数
- [x] 首页 Top 5 提示词排行：在首页新增排行卡片，展示最常用 Top 5
- [x] 首页 Top 5 提示词排行：点击排行条目可直接跳转 AI 生成器
- [x] 首页 Top 5 提示词排行：无使用历史时显示引导空状态

## v1.6 知识库管理模块
- [x] 数据库：新增 knowledge_docs 表（id、fileName、fileType、fileSize、s3Url、s3Key、extractedText、uploadedAt）
- [x] 后端 API：文档上传接口（接收 base64 文件，上传 S3，提取文本，存入数据库）
- [x] 后端 API：文档列表接口（返回所有已上传文档）
- [x] 后端 API：文档删除接口（删除 S3 文件 + 数据库记录）
- [x] 后端 API：知识库摘要接口（返回最新文档内容供 AI 使用）
- [x] 前端：新增「知识库」Tab 页面（文件上传 + 文档列表）
- [x] 前端：支持上传 PDF、TXT、DOCX 文件
- [x] 前端：文档列表显示文件名、大小、上传时间、状态
- [x] 前端：支持删除文档（确认弹窗）
- [x] AI 生成器：调用时自动注入最新知识库摘要作为上下文
